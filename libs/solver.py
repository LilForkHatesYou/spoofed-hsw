import tls_client, threading, asyncio
...
class Anty():
    ...
    def create_profile_and_send_id(self):
        ...
        response = self.session.post(
            'https://anty-api.com/browser_profiles',
            data=payload,
            headers={
...
class Browser():
    ...
    def setup(self):
        ...
        self.loop.run_until_complete(self.gotoDiscord())
        self.loop.run_until_complete(self.getIframHsw())
    
    def getHsw(self, rqTkn:str) -> str:
        with self.lock:
...
class Logger():
    ...
    def debug(self, text:str):
        if self.isDebug: print(f'[{Colors.purple}~{Colors.reset}] {text}')
...
class Solver():
    ...
    def predict(self) -> dict:
                try:
                    ...
                    task = requests.post(
                        f'https://{__CONFIG__["captcha-type"]}.nocaptchaai.com/solve',
                        headers={
                            'Content-type': 'application/json',
                            'apikey': self.apiKey
                        },
    ...
    def solve(self) -> str:
        ...
        #Get Captcha with a loop so if error it auto retry ^^
        while True:
        ...
        #Solve
        self.logger.debug(f'Solving: {self.question}')
        ...
        if captchaKey == None:
            ...
            self.logger.success(f'Solved Sucessfully In {round(time.time()-startedAs,2)}s: [{captchaKey[:30]}]')
            return captchaKey